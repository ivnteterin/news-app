...
function SourceList(props) {
  ...
  const createList = props.sources.map((source,i)=> {
    if (props.selectedSource === source.id || historySearch === source.id) {
      ...
      return <option selected id={source.id} value={i+1} key={uuidv4()}>{source.name}</option>
    }
    return <option id={source.id} value={i+1} key={uuidv4()}>{source.name}</option>
  });
  createList.unshift(<option value="DEFAULT" key={uuidv4()} >All sources</option>);
  ...
  return (
      ...
      <div className='sources-select'>
        {props.error ? "" : 
        <select defaultValue={"DEFAULT"} className='custom-select' id='source-filter' onChange={selectedSourceHandler}>{createList}</select>}     </div>
